...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        print(f"Epoch {epoch+1}/{args.epochs} proxy_det={proxy:.4f}")

    os.makedirs(args.out, exist_ok=True)
    with open(os.path.join(args.out, f"fasterrcnn_coco_{args.mixer}.json"), "w") as f:
        json.dump({"task":"coco_det_fasterrcnn","mixer":args.mixer,"proxy":float(proxy)}, f, indent=2)
